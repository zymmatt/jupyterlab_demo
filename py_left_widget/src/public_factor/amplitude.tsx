...
import { factor_prehandle_Widget } from "./factor_widgets";
import { factor_analysis_Widget } from "./factor_widgets";
import { factor_backtest_Widget } from "./factor_widgets";
const file_path = '/extension_dir/py_left_widget/command_work_file/';
const factor_name = '振幅'
//打开新的选项界面,预处理,因子分析,回测

/**
 * A Counter Lumino Widget that wraps a CounterComponent.
 */
 export class factor_amplitude_Widget extends ReactWidget {
	app_n: any;
	/**
				  ...
				  <div className="fctDesCon">
					  <section className="--jp-factor-header">
						  <p className="--jp-factor-title">
							  <span className="factor_name">公共因子：振幅（amplitude）</span>
							  <span className="storage_type">实时计算</span>
						  </p>
						  <p className="fct_info_item factor_type">
							  <span className="fct_info_label">因子类型：</span>
							  <span className="fct_info_value">Volatility</span>
						  </p>
						  <p className="fct_info_item">
							  <span className="fct_info_label">因子描述：</span>
							  <span className="fct_info_value">振幅 = （最高开收盘价格－最低开收盘价 ／ 最低开收盘价）－ 1。时间范围可设置为1周（5天），1个月（22天），3个月（66天），6个月（132天）（参数可调）。默认参数：1个月（22天）。</span>
						  </p>
						  <p className="fct_info_item">
							  <span className="fct_info_label">计算方法：</span>
							  <span className="fct_info_value">振幅 = （最高开收盘价格－最低开收盘价 ／ 最低开收盘价）－ 1。时间范围可设置为1周（5天），1个月（22天），3个月（66天），6个月（132天）（参数可调）。</span>
						  </p>
					  </section>
				  </div>
				  <div className="factorHandlerBar">
					  <div className="inner_wrapper">
						  <div className="fcthandlerItem factor_pretreat"
						  	onClick = {async()=>{
								const result = await showDialog({
									body: new factor_prehandle_Widget('振幅(amplitude)'),
								...
								if (result.button.label === "开始运行")  {
									...
									//commands.execute('fileeditor:run-all',temp)
						  ...
						  <div className="fcthandlerItem factor_analy"
							  onClick = {async()=>{
								  const result = await showDialog({
									  body: new factor_analysis_Widget('振幅(amplitude)'),
									  buttons: [Dialog.cancelButton({label: "取消"}), Dialog.okButton({label: "开始运行"})],
									  focusNodeSelector: "input",
									  title: "因子分析",
									});
						  ...
						  <div className="fcthandlerItem group_backtest"
						  	onClick = {async()=>{
								const result = await showDialog({
									body: new factor_backtest_Widget('振幅(amplitude)'),
									buttons: [Dialog.cancelButton({label: "取消"}), Dialog.okButton({label: "开始运行"})],
									focusNodeSelector: "input",
									title: "分组回测",
								  });
